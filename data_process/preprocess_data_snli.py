...
def create_train(dataset_dir, features_dir):
    ...
    hypothesis_lemma = []
    label = []
    with open(os.path.join(features_dir, 'premise_snli_1.0_train_token.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            premise.append(line)
    with open(os.path.join(features_dir, 'hypothesis_snli_1.0_train_token.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            hypothesis.append(line)

    with open(os.path.join(features_dir, 'premise_snli_1.0_train_lemma.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            premise_lemma.append(line)
    with open(os.path.join(features_dir, 'hypothesis_snli_1.0_train_lemma.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            hypothesis_lemma.append(line)

    with open(os.path.join(features_dir, 'label_snli_1.0_train.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            label.append(line)
    fw = open(os.path.join(dataset_dir, 'snli_train.txt'), mode='w')
...
def create_dev(dataset_dir, features_dir):
    ...
    hypothesis_lemma = []
    label = []
    with open(os.path.join(features_dir, 'premise_snli_1.0_dev_token.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            premise.append(line)
    with open(os.path.join(features_dir, 'hypothesis_snli_1.0_dev_token.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            hypothesis.append(line)

    with open(os.path.join(features_dir, 'premise_snli_1.0_dev_lemma.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            premise_lemma.append(line)
    with open(os.path.join(features_dir, 'hypothesis_snli_1.0_dev_lemma.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            hypothesis_lemma.append(line)

    with open(os.path.join(features_dir, 'label_snli_1.0_dev.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            label.append(line)
    fw = open(os.path.join(dataset_dir, 'snli_dev.txt'), mode='w')
    for i in range(len(premise)):
        fw.write(label[i] + '\t' + premise[i] + '\t' + hypothesis[i] + '\t' + premise_lemma[i]
                 + '\t' + hypothesis_lemma[i] + '\t' + str(i) + '\n')
    fw.close()
...
def create_test(dataset_dir, features_dir):
    ...
    hypothesis_lemma = []
    label = []
    with open(os.path.join(features_dir, 'premise_snli_1.0_test_token.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            premise.append(line)
    with open(os.path.join(features_dir, 'hypothesis_snli_1.0_test_token.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            hypothesis.append(line)

    with open(os.path.join(features_dir, 'premise_snli_1.0_test_lemma.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            premise_lemma.append(line)
    with open(os.path.join(features_dir, 'hypothesis_snli_1.0_test_lemma.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            hypothesis_lemma.append(line)

    with open(os.path.join(features_dir, 'label_snli_1.0_test.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            label.append(line)
    fw = open(os.path.join(dataset_dir, 'snli_test.txt'), mode='w')
    for i in range(len(premise)):
        fw.write(label[i] + '\t' + premise[i] + '\t' + hypothesis[i] + '\t' + premise_lemma[i]
                 + '\t' + hypothesis_lemma[i] + '\t' + str(i) + '\n')
    fw.close()
...
def create_lemma_embedding(dataset_dir, features_dir):
    ...
    fw = open(os.path.join(dataset_dir, 'snli_lemma_vocab.txt'), mode='w')
...
def create_word_embedding(dataset_dir, features_dir, glove_dir):
    ...
    with open(os.path.join(dataset_dir, 'snli_train.txt'), mode='r') as fd:
        for line in fd.readlines():
            ...
            for word in word_list:
                if (word not in vocab and word != ''):
                    vocab[word] = id
                    id += 1
    with open(os.path.join(dataset_dir, 'snli_dev.txt'), mode='r') as fd:
        for line in fd.readlines():
            array = line.lower().split('\t')
            word_list = array[1].split(' ')
            for word in word_list:
            ...
            for word in word_list:
                if (word not in vocab and word != ''):
                    vocab[word] = id
                    id += 1
    with open(os.path.join(dataset_dir, 'snli_test.txt'), mode='r') as fd:
        for line in fd.readlines():
            array = line.lower().split('\t')
            word_list = array[1].split(' ')
            for word in word_list:
    ...
    fw = open(os.path.join(dataset_dir, 'snli_glove_embedding.txt'), mode='w')
...
if __name__ == '__main__':
    ...
    dst_dir = os.path.join(base_dir, 'sequence_and_features_snli')
