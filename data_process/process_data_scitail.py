...
def build_sequence(filepath, dst_dir):
    ...
    with open(filepath) as f, \
        ...
        for line in f:
            sents = line.strip()
            array = json.loads(sents)
            if array['gold_label'] is '-':
                continue
            words_in = array['sentence1_binary_parse'].strip().split(' ')
            words_in = [x for x in words_in if x not in ('(', ')')]
            f1.write(' '.join(words_in) + '\n')
            len_p.append(len(words_in))

            words_in = array['sentence2_binary_parse'].strip().split(' ')
            words_in = [x for x in words_in if x not in ('(', ')')]
            f2.write(' '.join(words_in) + '\n')
            len_h.append(len(words_in))

            f3.write(dic[array['gold_label']] + '\n')
...
def create_train(dataset_dir, features_dir):
    ...
    hypothesis_lemma = []
    label = []
    with open(os.path.join(features_dir, 'premise_scitail_1.0_train_token.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            premise.append(line)
    with open(os.path.join(features_dir, 'hypothesis_scitail_1.0_train_token.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            hypothesis.append(line)

    with open(os.path.join(features_dir, 'premise_scitail_1.0_train_lemma.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            premise_lemma.append(line)
    with open(os.path.join(features_dir, 'hypothesis_scitail_1.0_train_lemma.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            hypothesis_lemma.append(line)

    with open(os.path.join(features_dir, 'label_scitail_1.0_train.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            label.append(line)
    fw = open(os.path.join(dataset_dir, 'scitail_train.txt'), mode='w')
...
def create_dev(dataset_dir, features_dir):
    ...
    hypothesis_lemma = []
    label = []
    with open(os.path.join(features_dir, 'premise_scitail_1.0_dev_token.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            premise.append(line)
    with open(os.path.join(features_dir, 'hypothesis_scitail_1.0_dev_token.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            hypothesis.append(line)

    with open(os.path.join(features_dir, 'premise_scitail_1.0_dev_lemma.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            premise_lemma.append(line)
    with open(os.path.join(features_dir, 'hypothesis_scitail_1.0_dev_lemma.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            hypothesis_lemma.append(line)

    with open(os.path.join(features_dir, 'label_scitail_1.0_dev.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            label.append(line)
    fw = open(os.path.join(dataset_dir, 'scitail_dev.txt'), mode='w')
    for i in range(len(premise)):
        fw.write(label[i] + '\t' + premise[i] + '\t' + hypothesis[i] + '\t' + premise_lemma[i]
                 + '\t' + hypothesis_lemma[i] + '\t' + str(i) + '\n')
    fw.close()
...
def create_test(dataset_dir, features_dir):
    ...
    hypothesis_lemma = []
    label = []
    with open(os.path.join(features_dir, 'premise_scitail_1.0_test_token.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            premise.append(line)
    with open(os.path.join(features_dir, 'hypothesis_scitail_1.0_test_token.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            hypothesis.append(line)

    with open(os.path.join(features_dir, 'premise_scitail_1.0_test_lemma.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            premise_lemma.append(line)
    with open(os.path.join(features_dir, 'hypothesis_scitail_1.0_test_lemma.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            hypothesis_lemma.append(line)

    with open(os.path.join(features_dir, 'label_scitail_1.0_test.txt'), mode='r') as fd:
        for line in fd.readlines():
            line = line.strip()
            label.append(line)
    fw = open(os.path.join(dataset_dir, 'scitail_test.txt'), mode='w')
    for i in range(len(premise)):
        fw.write(label[i] + '\t' + premise[i] + '\t' + hypothesis[i] + '\t' + premise_lemma[i]
                 + '\t' + hypothesis_lemma[i] + '\t' + str(i) + '\n')
    fw.close()
...
def create_lemma_embedding(dataset_dir, features_dir):
    ...
    fw = open(os.path.join(dataset_dir, 'scitail_lemma_vocab.txt'), mode='w')
...
def create_word_embedding(dataset_dir, features_dir, glove_dir):
    ...
    with open(os.path.join(dataset_dir, 'scitail_train.txt'), mode='r') as fd:
        for line in fd.readlines():
            ...
            for word in word_list:
                if (word not in vocab and word != ''):
                    vocab[word] = id
                    id += 1
    with open(os.path.join(dataset_dir, 'scitail_dev.txt'), mode='r') as fd:
        for line in fd.readlines():
            array = line.lower().split('\t')
            word_list = array[1].split(' ')
            for word in word_list:
            ...
            for word in word_list:
                if (word not in vocab and word != ''):
                    vocab[word] = id
                    id += 1
    with open(os.path.join(dataset_dir, 'scitail_test.txt'), mode='r') as fd:
        for line in fd.readlines():
            array = line.lower().split('\t')
            word_list = array[1].split(' ')
            for word in word_list:
    ...
    fw = open(os.path.join(dataset_dir, 'scitail_glove_embedding.txt'), mode='w')
...
if __name__ == '__main__':
    print('=' * 80)
    print('Preprocessing WordNet prolog and SciTail dataset')
    print('=' * 80)
    base_dir = os.path.dirname(os.path.realpath(__file__))
    dst_dir = os.path.join(base_dir, 'sequence_and_features_scitail')
    scitail_dir = os.path.join(base_dir, 'scitail/SciTailV1.1/snli_format')
    ...
    build_sequence(os.path.join(scitail_dir, 'scitail_1.0_dev.txt'), dst_dir)
    build_sequence(os.path.join(scitail_dir, 'scitail_1.0_test.txt'), dst_dir)
    build_sequence(os.path.join(scitail_dir, 'scitail_1.0_train.txt'), dst_dir)

    print('5. obtain lemma format for train/dev/test dataset\n')
    CoreNLP(os.path.join(dst_dir, 'premise_scitail_1.0_train.txt'))
    CoreNLP(os.path.join(dst_dir, 'hypothesis_scitail_1.0_train.txt'))
    CoreNLP(os.path.join(dst_dir, 'premise_scitail_1.0_dev.txt'))
    CoreNLP(os.path.join(dst_dir, 'hypothesis_scitail_1.0_dev.txt'))
    CoreNLP(os.path.join(dst_dir, 'premise_scitail_1.0_test.txt'))
    CoreNLP(os.path.join(dst_dir, 'hypothesis_scitail_1.0_test.txt'))

    print('6. build dictionary for word sequence and lemma sequence from training set\n')
    build_dictionary([os.path.join(dst_dir, 'premise_scitail_1.0_train_token.txt'),
                      os.path.join(dst_dir, 'hypothesis_scitail_1.0_train_token.txt')],
                     os.path.join(dst_dir, 'vocab_cased.pkl'), wordnet=word_id_num, remove_phrase=True)
    build_dictionary([os.path.join(dst_dir, 'premise_scitail_1.0_train_lemma.txt'),
                      os.path.join(dst_dir, 'hypothesis_scitail_1.0_train_lemma.txt')],
